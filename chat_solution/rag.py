...
class LearningAssistant:
    ...
    def _create_question_prompt(self, documents: str, query: str) -> str:
        ...
        new_context_str = f"New Context: {documents}" if documents else ""
...
Interaction 2
User input: 3
Assistant:Incorrect! LLMs generate responses by learning patterns, structures, and relationships in text from massive datasets. Try another topic!
...
Assistant:"""
    ...
    def call_llm(self, query: str) -> dict:
        ...
        if not query.isnumeric():
            print(f"Query is not numeric so we will use the context")
